...
function handleArrayOfGuesses(io, socket, lobbyCode, lobbies, guesses) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        (0, logger_1.log)(`[calculate score] ${lobbyCode}`);
            ...
            try {
                ...
                lobby.players.forEach((player) => __awaiter(this, void 0, void 0, function* () {
                    if (g.guess === 0 && player.id === g.player) {
                        ...
                        const curPlayerScoreIndex = (0, common_1.getCurrentPlayerScoreIndex)(lobbies, lobbyCode, player.id);
                        lobby.rounds[curRoundIndex].scores[curPlayerScoreIndex].score +=
                            common_1.VALUE_OF_TRUTH;
                        // increase player score
                        pointsUpdate = yield common_1.localAxios.put(`/api/score/increase/${player.pid}`, {
                            game_id,
                            points: common_1.VALUE_OF_TRUTH,
                        });
                        (0, logger_1.log)(`+${common_1.VALUE_OF_TRUTH} truth player : ${player.username}`);
                        ...
                        const curPlayerScoreIndex = (0, common_1.getCurrentPlayerScoreIndex)(lobbies, lobbyCode, player.id);
                        lobby.rounds[curRoundIndex].scores[curPlayerScoreIndex].score +=
                            common_1.VALUE_OF_BLUFF;
                        // increase player score
                        pointsUpdate = yield common_1.localAxios.put(`/api/score/increase/${player.pid}`, {
                            game_id,
                            points: common_1.VALUE_OF_BLUFF,
                        });
                        (0, logger_1.log)(`+${common_1.VALUE_OF_BLUFF} Bluff player : ${player.username}`);
                        ...
                        (0, logger_1.log)(`+${common_1.VALUE_OF_TRUTH} definition : ${player.definitionID}`);
                        // log(pointsUpdate.data);
                ...
                if (err instanceof Error) {
                    (0, logger_1.log)(`error: handleArrayOfGuesses, ${err.message}`);
                }
            }
        }));
        try {
            ...
            if (newRound.status === 200) {
                (0, logger_1.log)(`* end of round ${roundId}`);
            }
        }
        catch (err) {
            (0, logger_1.log)(`error while ending round!, ${lobbyCode}`);
            (0, handleErrorMessage_1.default)(io, socket, 2003, "there was a server error while ending the round");
            return;
        }
        (0, logger_1.log)(`changing phase, ${lobbyCode} -> POSTGAME`);
